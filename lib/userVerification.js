...
async function saveUserVerificationIfNotExists({ _id, email, identifier }) {
  try {
    const existingRecord = await UserVerificationModel.findOne({ email: email });
    ...
    if (!existingRecord) {
      ...
      await UserVerificationModel.updateOne({ _id: existingRecord._id }, { identifier: identifier });
